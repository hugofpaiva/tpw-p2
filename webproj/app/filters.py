...
class ListFilter(filters.Filter):
    def filter(self,qs,value):
        if value not in (None,''):
            strings = [str(v) for v in value.split(',')]
...
class ProductFilter(filters.FilterSet):
    ...
    category = ListFilter(field_name='category__title', lookup_expr='in', exclude='')
    developer = ListFilter(field_name='developer__name',lookup_expr='in')
